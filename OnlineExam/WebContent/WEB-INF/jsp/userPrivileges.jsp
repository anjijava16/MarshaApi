        ...
        <form action="removeUserE.do"> 
         <table cellspacing="0" cellpadding="15" border="0" width="600" >
             <tr style="background-color: #a9a8f0; font-size: 20px; font-weight: bold">
              <td>UserID</td>
             <td><b>Email</b></td>
             <td><b>CurrentStatus</b></td>
             </tr>
	<c:forEach items="${listOfUsers}" var="exam">
		<tr>
		<td><c:out value="${exam.username}"></c:out></td>
		<td><c:out value="${exam.emailId}"></c:out></td>
		 <c:if test="${exam.roleType=='ADMIN'}">  
		<td><a href="" value=value="<c:out value="${exam.username}"/>"><img src="${pageContext.request.contextPath}/images/admin-user-toggle.png" style="border: 0"/></td></a>
		</c:if>
		<c:if test="${exam.roleType!='ADMIN'}">  
		<td><a href="" value=value="<c:out value="${exam.username}"/>"><img src="${pageContext.request.contextPath}/images/normal-user-toggle.png" style="border: 0"/></td></a>
		</c:if>
		</tr>
</c:forEach>
</table>
